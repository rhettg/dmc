...
# -*- coding: utf-8 -*-
"""Functions for displaying dates and times humanely

The `humanize` module is great, except it has no understanding of timezone
issues.  Also it can't respect our test hooks, so we'll just copy it in here.
"""
from humanize.i18n import ngettext, gettext as _
import datetime

__all__ = ['naturaldelta', 'naturaltime', 'naturalday', 'naturaldate']


def _now(tz=None, local=False):
    from .time import Time
    return Time.now().to_datetime(tz=tz, local=local).replace(tzinfo=None)


def _today(tz=None, local=False):
    from .time import Time
    from .date import Date
    return Date.from_time(Time.now(), tz=tz, local=local).to_datetime_date()
...
def naturaltime(value, future=False, months=True):
    ...
    # dmc note: naturaltime is just relative, so we assume we're dealing with
    # UTC
    if now.tzinfo != value.tzinfo:
        raise ValueError("mismatched tzinfo")
...
def naturalday(value, format='%b %d', tz=None, local=None):
    ...
    try:
        ...
        return value
    delta = value - _today(tz=tz, local=local)
    if delta.days == 0:
        return _('today')
    elif delta.days == 1:
        return _('tomorrow')
...
def naturaldate(value, tz=None, local=False):
    ...
    try:
        ...
        # Date arguments out of range
        return value
    delta = abs_timedelta(value - _today(tz=tz, local=local))
    if delta.days >= 365:
        return naturalday(value, '%b %d %Y', tz=tz, local=local)
    return naturalday(value, tz=tz, local=local)
